...
namespace AbpCachingPlayground.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("AbpCachingPlayground") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "AbpCachingPlayground", DisplayName = "AbpCachingPlayground API", Resources = { "AbpCachingPlayground" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["AbpCachingPlayground_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["AbpCachingPlayground_Web:RootUrl"]!.EnsureEndsWith('/');

            /* AbpCachingPlayground_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["AbpCachingPlayground_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        var consoleAndAngularClientId = configurationSection["AbpCachingPlayground_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["AbpCachingPlayground_App:RootUrl"]?.TrimEnd('/');
            await CreateApplicationAsync(
                applicationType: OpenIddictConstants.ApplicationTypes.Web,
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["AbpCachingPlayground_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["AbpCachingPlayground_Swagger:RootUrl"]?.TrimEnd('/');
